...
contract FilmChainNFT is ERC721URIStorage, Ownable, ReentrancyGuard {
    ...
    event AuctionCancelled(uint256 indexed tokenId, address indexed creator);
    ...
    function createNFT(
        ...
        require(!isAuction || auctionDuration > 0, "Invalid auction duration");
    ...
    function placeBid(uint256 tokenId, uint256 bidAmount) public nonReentrant {
        ...
        require(block.timestamp < item.auEndTime, "Auction has ended");
    ...
    function cancelAuction(uint256 tokenId) public {
        NFTItem storage item = nftItems[tokenId];
        require(item.isAuction, "Item is not on auction");
        require(msg.sender == item.creator, "Only the creator can cancel the auction");
        require(block.timestamp < item.auEndTime, "Auction has already ended");
        require(item.highestBidder == address(0), "Cannot cancel an auction with bids");
