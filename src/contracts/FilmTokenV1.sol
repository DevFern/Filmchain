...
contract FilmToken is ERC20, Ownable {
    ...
    function fundProject(uint256 projectId, uint256 amount) public returns (bool) {
    ...
    function purchaseNFT(uint256 nftId, uint256 amount) public returns (bool) {
        require(amount > 0, "Amount must be greater than 0");
        require(balanceOf(msg.sender) >= amount, "Insufficient balance");
    ...
    function distributeRewards(address recipient, uint256 projectId, uint256 amount) public onlyOwner returns (bool) {
        require(amount > 0, "Amount must be greater than 0");
